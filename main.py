...
print(' SELAMAT DATANG DI DUNIA TIPU TIPU ' )
...
if sandi == 'bang' and user == 'bangbro':

	print "Kamu Berhasil Mlebu"
...
def log(value, status='BERBURU KOPI', color='[CC]'):
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('[R1]KOPI PLUS MINGGIR '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('[G1] KAPAL API MASUK ''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
     ...
     '[Y1]                   BANG BRO 30                ','[CC]'
     '[G1]              YOUTUBE : ORA GABLEGG     ','[CC]'
     '[G1]               STATUS SEDANG TIDUR   ','[CC]'
     '[P1]       ','[CC]'
     '[Y1]               TAK ADA YANG ABAADI   ','[CC]'
     '[Y1]          DONT DIE BEFORE YOU DEATH OKE ','[CC]'
     '[G1] ','[CC]'
